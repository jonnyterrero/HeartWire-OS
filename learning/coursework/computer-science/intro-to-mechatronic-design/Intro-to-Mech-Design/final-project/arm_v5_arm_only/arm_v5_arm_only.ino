...
// Servos: hand=D1*, wrist=D5, forearm=D6, shoulder=D10 | RGB R,G,B = 7,8,9 | photo A3
// 74HC595: latch=2, data=3, clk=4  |  *D1 = UART TX — keep Serial light during grip
//
// For pins 4,7,10,12 (like working_arm_code_final), edit HAND/WRIST/FOREARM/SHOULDER below and
// move red LED off D7 (e.g. D11) if wrist uses D7.
...
int avgRead() {
  ...
  for (uint8_t i = 0; i < 10; i++) {
    total += analogRead(PIN_PHOTO);  // required — without this, color is always 0 and the FSM misbehaves
    delay(5);
  }
  return (int)(total / 10);
}
