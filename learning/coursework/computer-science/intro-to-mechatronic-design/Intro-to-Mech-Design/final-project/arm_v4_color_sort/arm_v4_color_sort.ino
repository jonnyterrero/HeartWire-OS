...
// Squared-distance threshold above which the closest saved color is
// considered "too far" -> treated as No Ball. Tune empirically:
//   - Set to a large number (e.g. 999999L) to disable.
//   - Read distances logged in readColorStable(); a typical match is
//     under a few thousand for a clean ball, much higher for empty pad.
const long NO_BALL_DIST_THRESHOLD = 8000L;
...
int bestMatch(int x[3], long &bestDOut) {
  int  best  = 0;
...
void readColorStable() {
  ...
  while (attempt < MAX_COLOR_ATTEMPTS) {
    ...
    getColor(now);
    long bestD;
    int match = bestMatch(now, bestD);

    // ISS-02 guard: if the closest saved color is too far away, the pad
    // is likely empty / lighting changed -> demote to No Ball.
    if (match != 4 && bestD > NO_BALL_DIST_THRESHOLD) {
      Serial.print("  try "); Serial.print(attempt + 1);
      Serial.print(" too far d="); Serial.print(bestD);
      Serial.println(" -> No Ball");
      match = 4;
    } else {
      Serial.print("  try "); Serial.print(attempt + 1);
      Serial.print(" -> "); Serial.print(names[match]);
      Serial.print(" d="); Serial.print(bestD);
      Serial.print(" [G,R,B]=");
...
// ===================== TEACH MODE =====================
//
// Drive each joint by Serial keystroke. Captures pose lines you can
// paste back into the constants block. Activate by typing 'j' at the
// "ready" prompt in setup() instead of any other character.
//
// Keys (single character + Enter, or just stream characters):
//   a / d   elbow   -/+ step
//   w / s   forearm +/- step
//   r / f   wrist   +/- step  (r=raise, f=forward/down)
//   o / c   hand    open/close
//   +/-     step    bigger/smaller (default 2 deg, range 1..15)
//   p       print   current pose as a copy-pasteable comment
//   h       home    pose
//   q       quit    teach mode and continue boot

void printTeachPose() {
  Serial.print("// elbow=");   Serial.print(posElbow);
  Serial.print(" forearm=");   Serial.print(posForearm);
  Serial.print(" wrist=");     Serial.print(posWrist);
  Serial.print(" hand=");      Serial.println(posHand);
}

void teachMove(Servo &s, int &pos, int target) {
  target = constrain(target, 0, 180);
  moveServoSmooth(s, pos, target);
}
...
void teachMode() {
  ...
  Serial.println("a/d elbow  w/s forearm  r/f wrist  o/c hand");
  Serial.println("+/- step   p print   h home   q quit");
  printTeachPose();
  ...
  while (true) {
    ...
    char k = Serial.read();
    switch (k) {
      case 'a': teachMove(elbow,   posElbow,   posElbow   - step); break;
      case 'd': teachMove(elbow,   posElbow,   posElbow   + step); break;
      case 'w': teachMove(forearm, posForearm, posForearm + step); break;
      case 's': teachMove(forearm, posForearm, posForearm - step); break;
      case 'r': teachMove(wrist,   posWrist,   posWrist   + step); break;
      case 'f': teachMove(wrist,   posWrist,   posWrist   - step); break;
      case 'o': teachMove(hand,    posHand,    HAND_OPEN);         break;
      case 'c': teachMove(hand,    posHand,    HAND_CLOSED);       break;
      case '+': step = min(step + 1, 15); Serial.print("step="); Serial.println(step); break;
      case '-': step = max(step - 1, 1);  Serial.print("step="); Serial.println(step); break;
      case 'p': printTeachPose(); break;
      case 'h':
        moveToPose(CARRY_FOREARM, CARRY_WRIST);
        ...
        break;
      case 'q':
        Serial.println("Exiting teach mode.");
        return;
      case '\n': case '\r': case ' ': break;
      default:
...
void setup() {
  ...
  Serial.println("Type 'j' for teach/jog mode, or any other key to begin color calibration.");
  while (!Serial.available()) {}
