...
// #region agent log
// NDJSON-style one line per event; prefix #DBG so you can copy lines into debug-8e07e1.log
static void agentLog(const char* hypothesisId, const char* location, const char* message,
                     int cmdOrNeg1, int hand, int ho, int hc) {
  Serial.print(F("#DBG{\"sessionId\":\"8e07e1\",\"timestamp\":"));
  Serial.print((unsigned long)millis());
  Serial.print(F(",\"runId\":\"pre\",\"hypothesisId\":\""));
...
void setup() {
  ...
  // #region agent log
  agentLog("B", "setup", "after_load", -1, (int)currentPose.hand, (int)HAND_OPEN, (int)HAND_CLOSED);
  // #endregion
...
void calibrateOnePose(const __FlashStringHelper* label, ArmPose &targetPose) {
  ...
  while (!savedPosition) {
    if (Serial.available()) {
      ...
      if (cmd == '\n' || cmd == '\r') continue;
      // #region agent log
      agentLog("C", "calibrateOnePose", "raw_cmd", (int)(unsigned char)cmd, (int)currentPose.hand,
               (int)HAND_OPEN, (int)HAND_CLOSED);
      // #endregion
      ...
      // #region agent log
      if (cmd == 'o' || cmd == 'c') {
        agentLog("A", "calibrateOnePose", "after_oc", (int)(unsigned char)cmd, (int)currentPose.hand,
                 (int)HAND_OPEN, (int)HAND_CLOSED);
      }
      if (cmd != 'e' && cmd != 'E' && cmd != 'f' && cmd != 'F' && cmd != 'w' && cmd != 'W' &&
          cmd != 'o' && cmd != 'O' && cmd != 'c' && cmd != 'C' && cmd != 's' && cmd != 'S') {
        agentLog("F", "calibrateOnePose", "unrecognized", (int)(unsigned char)cmd, (int)currentPose.hand,
                 (int)HAND_OPEN, (int)HAND_CLOSED);
      }
      // #endregion
...
void moveToPose(ArmPose p) {
  // #region agent log
  static bool s_handLogInit = false;
  static byte s_lastHandLogged = 0;
  if (!s_handLogInit || s_lastHandLogged != p.hand) {
    s_handLogInit     = true;
    s_lastHandLogged  = p.hand;
    agentLog("D", "moveToPose", "hand_write", -1, (int)p.hand, (int)HAND_OPEN, (int)HAND_CLOSED);
  }
  // #endregion
